...
@author: Kevin Hennessey, Dylan Pryor
"""
import pandas as pd
import numpy as np
...
features = pd.read_csv("C:/Users/Dylan/Documents/SP21/CSE 5243/data_comp/train_features.csv")
demos = pd.read_csv("C:/Users/Dylan/Documents/SP21/CSE 5243/data_comp/train_demos.csv")
labels = pd.read_csv("C:/Users/Dylan/Documents/SP21/CSE 5243/data_comp/train_labels.csv")
...
X_train, X_valid, y_train, y_valid = train_test_split(full_df.iloc[:, :-1], full_df['mortality'], test_size=0.2,
                                                      random_state=1)
...
# %% Classifier #4 (decision tree ensemble bagging) (not great: AUC = 0.75)
bagg = BaggingClassifier(n_estimators=15, max_samples=15)
bagg.fit(X_train, y_train)
y_pred_bagg = bagg.predict(X_valid)
bagg_prob = bagg.predict_proba(X_valid)

results_function(y_valid, y_pred_bagg, "Ensemble Bagging (D-trees)", bagg)
roc(y_valid, bagg_prob, "Ensemble Bagging (D-trees)")
disp_bagg = metrics.plot_confusion_matrix(bagg, X_valid, y_valid, cmap=plt.cm.Blues)
disp_bagg.ax_.set_title("Ensemble Bagging (D-trees)")
plt.show()

# %% Classifier #5 (naive bayes ensemble bagging (multinomial)) (not great: AUC = 0.73)
bagg = BaggingClassifier(base_estimator=MultinomialNB(),n_estimators=10)
bagg.fit(X_train, y_train)
y_pred_bagg = bagg.predict(X_valid)
bagg_prob = bagg.predict_proba(X_valid)

results_function(y_valid, y_pred_bagg, "Ensemble Bagging (M-NB)", bagg)
roc(y_valid, bagg_prob, "Ensemble Bagging (M-NB)")
disp_bagg = metrics.plot_confusion_matrix(bagg, X_valid, y_valid, cmap=plt.cm.Blues)
disp_bagg.ax_.set_title("Ensemble Bagging (M-NB)")
plt.show()

# %% Classifier #6 (naive bayes ensemble bagging (gaussian)) (not great: AUC = 0.75)
bagg = BaggingClassifier(base_estimator=GaussianNB(),n_estimators=10)
bagg.fit(X_train, y_train)
y_pred_bagg = bagg.predict(X_valid)
bagg_prob = bagg.predict_proba(X_valid)

results_function(y_valid, y_pred_bagg, "Ensemble Bagging (G-NB)", bagg)
roc(y_valid, bagg_prob, "Ensemble Bagging (G-NB)")
disp_bagg = metrics.plot_confusion_matrix(bagg, X_valid, y_valid, cmap=plt.cm.Blues)
disp_bagg.ax_.set_title("Ensemble Bagging (G-NB)")
plt.show()
